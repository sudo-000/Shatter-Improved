...
def sh_import_segment(fp, context, compressed=False):
		...
		for d in drm:
			if (d == "NoImport" or d == "no_import"):
				show_message("Import error",
							 "The creator of this segment has requested that it not be imported. While you could bypass this, we encourage you to respect this request.")
				return {"FINISHED"}
	...
	if (segattr.get("meshbake_lightFactor") or segattr.get("meshbake_disableLight")):
		warnings.add(
			"legacy meshbake tags were detected which means exported segments will look significantly different when exported with the current version of Shatter")
	...
	for obj in root:
		...
		if (properties.get("type") == "stone"):
			warnings.add(
				"an obstacle was ignored becuase it had a type of 'stone' but it didn't have the 'IMPORT_IGNORE' or 'shbt-ignore' attribute set")
			continue
		...
		# Obstacles
			...
			if (scene.sh_default_template and
					(
							(o.sh_properties.sh_obstacle.startswith(
								"score") and o.sh_properties.sh_template == f"{scene.sh_default_template}_st")
							or
							(not o.sh_properties.sh_obstacle.startswith(
								"score") and o.sh_properties.sh_template == f"{scene.sh_default_template}_glass")
					)
			):
		...
		# Decals
			...
			if (colour):
				...
				colour = colour.split()
				colour = (
				float(colour[0]), float(colour[1]), float(colour[2]), float(colour[3]) if len(colour) == 4 else 1.0)
	...
	if (len(warnings)):
		...
		if (context.preferences.addons["shatter"].preferences.enable_segment_warnings):
			show_message("Import warnings",
						 f"The segment imported successfully, but some possible issues were noticed: {warnlist}.")
