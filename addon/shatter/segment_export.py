...
class ExportWarnings():
	...
	def display(self):
		...
		if (len(self.warnings) and bpy.context.preferences.addons["shatter"].preferences.enable_segment_warnings):
			...
			butil.show_message("Export warnings",
							   f"The segment exported successfully, but some possible issues were noticed: {warnlist}.")
...
def sh_create_root(scene, params):
	...
	if (scene.sh_auto_length):
		...
		for o in bpy.data.objects:
			if ((-o.location[0]) / 2 + o.dimensions[0] / 4 > sizeZ):
				sizeZ = candZ
...
def make_subelement_from_entity(level_root, scene, obj, params):
	...
	if (sh_type == "OBS"):
		properties[
			"type"] = obj.sh_properties.sh_obstacle_chooser if obj.sh_properties.sh_use_chooser else obj.sh_properties.sh_obstacle
	...
	if (sh_type == "OBS" and obj.sh_properties.sh_param0 and obj.sh_properties.sh_template):
		params["warnings"].add(
			"both param0 and a template are set on some obstacles making param0 override the template - since param0 is often used for colours this might result in clear glass")
	...
	if (sh_type == "BOX"):
		...
		if (not obj.sh_properties.sh_use_multitint):
			...
			if (obj.sh_properties.sh_tint[0] != 1.0 or obj.sh_properties.sh_tint[1] != 1.0 or obj.sh_properties.sh_tint[
				2] != 1.0):
				properties["color"] = exportList(obj.sh_properties.sh_tint)
		else:
			properties["color"] = str(obj.sh_properties.sh_tint1[0]) + " " + str(
				obj.sh_properties.sh_tint1[1]) + " " + str(obj.sh_properties.sh_tint1[2]) + " " + str(
				obj.sh_properties.sh_tint2[0]) + " " + str(obj.sh_properties.sh_tint2[1]) + " " + str(
				obj.sh_properties.sh_tint2[2]) + " " + str(obj.sh_properties.sh_tint3[0]) + " " + str(
				obj.sh_properties.sh_tint3[1]) + " " + str(obj.sh_properties.sh_tint3[2])
		...
		if (not obj.sh_properties.sh_use_multitile):
			...
			properties["tile"] = str(obj.sh_properties.sh_tile1) + " " + str(obj.sh_properties.sh_tile2) + " " + str(
				obj.sh_properties.sh_tile3)

		# Tile size for boxes
		if (obj.sh_properties.sh_tilesize[0] != 1.0 or obj.sh_properties.sh_tilesize[1] != 1.0 or
				obj.sh_properties.sh_tilesize[2] != 1.0):
			properties["tileSize"] = exportList(obj.sh_properties.sh_tilesize)

		# Tile rotation
		if (obj.sh_properties.sh_tilerot[1] > 0.0 or obj.sh_properties.sh_tilerot[2] > 0.0 or
	...
	if (params.get("sh_box_bake_mode", "Mesh") == "StoneHack" and sh_type == "BOX" and (
			obj.sh_properties.sh_visible or use_legacy)):
		"""
...
def createSegmentText(scene, params):
	...
	file_header = "<!-- Exporter: Shatter for Blender " + str(common.BL_INFO["version"][0]) + "." + str(
		common.BL_INFO["version"][1]) + "." + str(common.BL_INFO["version"][2]) + " -->\n"
...
def sh_export_segment_ext(filepath, context, scene, compress=False, params={}):
	...
	if (filepath == None and params.get("auto_find_filepath", False)):
		...
		if (not filepath):
			butil.show_message("Export error",
							   "There is currently no APK open in APK Editor Studio or your asset override path isn't set. Please open a Smash Hit APK with a valid structure or set an asset path in Shatter settings and try again.")
			return {"FINISHED"}

		if ((not props.sh_level or not props.sh_room or not props.sh_segment) and (not props.sh_segment)):
			butil.show_message("Export error",
							   "You have not set one of the level, room or segment name properties needed to use auto export to apk feature. Please set these in the scene tab and try again.")
			return {"FINISHED"}

		segstrate_path = filepath + "/slk" if ospath.exists(filepath + "/slk") else None

		# Real file path
		filepath += "/segments/" + props.sh_level + "/" + props.sh_room + "/" + props.sh_segment + (
			".xml.gz.mp3" if compress else ".xml.mp3")
	...
	if (params.get("sh_test_server", False) == True):
		...
		if (params.get("sh_box_bake_mode", "Mesh") == "Mesh"):
			...
			bake_mesh.bakeMeshToFile(content, tempdir + "/segment.mesh", templates,
									 bake_mesh.BakeProgressInfo(MB_progress_update_callback))
	...
	if (params.get("sh_box_bake_mode", "Mesh") == "Mesh"):
		...
		# Bake mesh
		bake_mesh.bakeMeshToFile(content, meshfile,
								 (params["sh_meshbake_template"] if params["sh_meshbake_template"] else None),
								 bake_mesh.BakeProgressInfo(MB_progress_update_callback))
