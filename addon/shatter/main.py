...
class sh_static_segstrate(bpy.types.Operator, ImportHelper):
	...
	def execute(self, context):
		if (self.agreement):
			...
			butil.show_message("Segstrate error",
							   "The agreement has not been accepted and the protection has not been preformed.")
...
class sh_SceneProperties(PropertyGroup):
	...
	sh_box_bake_mode: EnumProperty(
		...
		items=[
			...
			('StoneHack', "Obstacle",
			 "Adds a custom obstacle named 'stone' for every box that attempts to simulate stone. Only colour is supported: there are no textures"),
	...
	sh_powerup: EnumProperty(
		...
		items=[
			...
			('antigravity', "Anti-gravity*",
			 "*Does not work anymore. Old power up that probably would have reversed gravity"),
...
class sh_AddonPreferences(AddonPreferences):
	...
	updater_channel: EnumProperty(
		...
		items=[
			('stable', "Normal",
			 "Contains new updates and features as soon as they are available, but might also break sometimes."),
		...
		if (t == "BOX"):
			...
			for x in ["tint", "tile"]:
				...
				if (ui.get(f"sh_use_multi{x}")):
					ui.prop(f"sh_use_multi{x}", text="Per-axis", text_compact=f"Per-axis {word.lower()}",
							use_button=True)
					ui.prop(f"sh_{x}1")
					ui.prop(f"sh_{x}2")
					ui.prop(f"sh_{x}3")
...
class SHATTER_MT_3DViewportMenu(Menu):
	...
	def draw(self, context):
		...
		for t in [("box", "MESH_CUBE"), ("obstacle", "MESH_CONE"), ("decal", "TEXTURE"), ("powerup", "SOLO_OFF"),
				  ("water", "MATFLUID")]:
...
class AutogenProperties(PropertyGroup):
	...
	type: EnumProperty(
		...
		items=[
			...
			('SingleRow', "Row of boxes",
			 "A single row of boxes, often used before and after chasms. Look at the first room of the game for an example of this"),
	...
	algorithm: EnumProperty(
		...
		items=[
			...
			('ArithmeticProgressionSet', "ArithmeticProgressionSet",
			 "Randomly selected from a subset of a arithmetic series (ex: random of 1/2, 1/4, 1/6)"),
			('GeometricProgressionSet', "GeometricProgressionSet",
			 "Randomly selected from a subset of a geometric series (ex: random of 1/2, 1/4, 1/8)"),
		],
		...
		if (props.type == "SingleRow"):
			...
			if (props.algorithm in ["GeometricProgressionSet", "ArithmeticProgressionSet"]):
				...
				sub.prop(props, "geometric_exponent_minmax",
						 text="Exponent" if props.algorithm.startswith("G") else "Scalar")
...
class BlenderPlacer:
	...
	def setBase(self, basePos, baseSize):
		...
		self.base = autogen.Box(autogen.Vector3(basePos[1], basePos[2], basePos[0]),
								autogen.Vector3(baseSize[1] / 2, baseSize[2] / 2, baseSize[0] / 2))
	...
	def inheritProperties(self, obj, template_append=""):
		...
		if (hasattr(self, "base")):
			update_properties = ["sh_visible", "sh_template", "sh_tint", "sh_use_multitint", "sh_tint1", "sh_tint2",
								 "sh_tint3", "sh_tile", "sh_use_multitile", "sh_tile1", "sh_tile2", "sh_tile3",
								 "sh_tilerot", "sh_tilesize"]
	...
	def addBox(self, box):
		...
		# Add the mesh
		bpy.ops.mesh.primitive_cube_add(size=1.0, location=(box.pos.z, box.pos.x, box.pos.y),
										scale=(box.size.z * 2, box.size.x * 2, box.size.y * 2))
...
class RunAutogenAction(bpy.types.Operator):
	...
	def execute(self, context):
		...
		placer = BlenderPlacer(
			...
			context.object if context.object and (
						context.object.sh_properties.sh_visible or not props.template) else props.template,
		)
		...
		if (props.type == "SingleRow"):
			...
			if (not placer.getBase()):
				butil.show_message("Shatter Autogen error",
								   "To use the single row generator, please select a box to build on top of.")
				return {"FINISHED"}
