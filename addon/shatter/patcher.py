...
def _patch_v142_v143_arm64_savekey(patcher, params):
	...
	if (not value):
		msg.append(
			"The encryption key will be set to Smash Hit's default key, 5m45hh1t41ght, since you did not set one.")
		value = "5m45hh1t41ght"
...
def _patch_v142_v143_arm64_dropballs(patcher, params):
	...
	if (not value):
		return [
			"You didn't put in a value for how many balls you want to drop when you hit on something. Dropping balls won't be patched!"]
...
def _main():
	...
	if (result == NotImplemented):
		print(
			"Error: Either you specified an invalid patch (most likely) or this version and archiecture combination are not supported by the patch tool!")
	elif (result):
